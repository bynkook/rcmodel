"""
train_stack8.py
Scikit-learn StackingRegressor 기반 스태킹 학습 + Optuna 파라미터 탐색.
"""
...
def train_per_target_with_optuna(
    ...
    n_trials: int = 30
) -> Tuple[Dict[str, Pipeline], Dict[str, Dict[str, str]]]:
    
    """타깃별 Optuna → 최적 파라미터로 full train 파이프라인 학습"""
...
def main():    
    ...
    bundle = {
        "models": models,                      # {tgt: Pipeline}
        "features_by_target": feats_by_tgt,    # {tgt: [feat,...]}
        "dtypes_by_target": dtypes_by_tgt,     # {tgt: {feat: dtype_str}}
        "targets": list(models.keys()),        # [tgt,...]
        "best_params_by_target": best_params_by_tgt,    # {tgt: [parameters,...]}
    ...
    for tgt in models:
        ...
        y_pred = models[tgt].predict(X_te)
