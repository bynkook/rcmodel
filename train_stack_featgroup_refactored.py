...
def build_preprocessor(X: pd.DataFrame) -> ColumnTransformer:
    ...
    - numeric: MinMax
    - log(=phi_mn): log1p + MinMax
    - exp(=rho)   : exp + MinMax  (사용자 기존 의도 유지)
    - categorical: OneHot
    """
...
def generate_and_save_reports(
    ...
    for tgt, model in models.items():
        ...
        print(
            f"[{tgt:10s}] "
            f"Train MAE={tr_metrics['MAE']:.6f} RMSE={tr_metrics['RMSE']:.6f} R2={tr_metrics['R2']:.6f} | "
            f"Test  MAE={te_metrics['MAE']:.6f} RMSE={te_metrics['RMSE']:.6f} R2={te_metrics['R2']:.6f}"
        )
